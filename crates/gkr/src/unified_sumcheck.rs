...
//! Unified FROST-GKR sumcheck.
//!
//! Discharges the full 59-permutation algebraic surface
//! (C1 + β·C1' + γ·C2)
//! in a single degree-9 sumcheck of length `N_UNIFIED_VARS = 15`.
//!
...
//! columns by the shift gadget. `state(r')` is
...
//! Block128 multiplications in total for the verifier.
...
use frost_gkr_core::packed::pow7::{pow7_block128, pow7_flat_block128};
use frost_gkr_core::sumcheck::RoundPolynomial;
...
use crate::unified_mle::{
    UnifiedMle, N_LANE_VARS, N_ROUND_VARS, N_SLOT_VARS, N_UNIFIED_CELLS, N_UNIFIED_VARS,
};
...
const ELEM_HI: usize = ELEM_LO + N_LANE_VARS;
const ROUND_LO: usize = ELEM_HI;
const ROUND_HI: usize = ROUND_LO + N_ROUND_VARS;
const SLOT_LO: usize = ROUND_HI;
const SLOT_HI: usize = SLOT_LO + N_SLOT_VARS;
...
pub const UNIFIED_ROUND_DEGREE: usize = 9;
...
pub struct UnifiedProof {
    /// `N_UNIFIED_VARS` round polynomials, each of degree at
    /// most `UNIFIED_ROUND_DEGREE`.
...
/// point plus the cross-checked witness claims consumed by the terminal
/// batch-evaluation arguments.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct UnifiedReduction {
...
/// Run the unified main sumcheck on a populated `UnifiedMle`.
///
...
pub fn prove_unified_sumcheck<T: FiatShamir<Block128>>(
    mle: &UnifiedMle,
    channel: &mut T,
) -> (UnifiedProof, Vec<Block128>) {
    assert_eq!(mle.s_in.len(), N_UNIFIED_CELLS);
    assert_eq!(mle.s_out.len(), N_UNIFIED_CELLS);
    assert_eq!(mle.sigma.len(), N_UNIFIED_CELLS);
    assert_eq!(mle.state.len(), N_UNIFIED_CELLS);

    let rho: Vec<Block128> = (0..N_UNIFIED_VARS).map(|_| channel.squeeze()).collect();
    ...
    let mut round_polys = Vec::with_capacity(N_UNIFIED_VARS);
    let mut r_prime = vec![Block128::ZERO; N_UNIFIED_VARS];

    for round in 0..N_UNIFIED_VARS {
        ...
        r_prime[N_UNIFIED_VARS - 1 - round] = challenge;
...
pub fn verify_unified_sumcheck<T: FiatShamir<Block128>>(
    proof: &UnifiedProof,
    channel: &mut T,
) -> Option<UnifiedReduction> {
    if proof.round_polys.len() != N_UNIFIED_VARS {
        return None;
    }
    for p in &proof.round_polys {
        if p.degree() > UNIFIED_ROUND_DEGREE {
            return None;
        }
    }
...
fn build_unified_tables(mle: &UnifiedMle, rho: &[Block128]) -> UnifiedTables {
    ...
    let mds_lane_dec: [Vec<Block128>; STATE_SIZE] = std::array::from_fn(build_mds_lane_table);
...
/// this version is kept as a parity oracle for the
...
fn compute_round_polynomial(
    ...
    let mut evals = [Block128::ZERO; UNIFIED_ROUND_DEGREE + 1];
...
fn t_flat_table() -> [u128; UNIFIED_ROUND_DEGREE + 1] {
    std::array::from_fn(|k| tower_to_flat_u128(k as u128))
}

/// Per-evaluation-point flat-basis reference prover.
///
/// Kept as a parity oracle for the monomial-form prover
/// `compute_round_polynomial_flat`; the active prover uses the monomial form.
#[allow(dead_code)]
fn compute_round_polynomial_flat_per_eval(
    ...
    let half = tabs.u.len() / 2;
    let mut evals = [0u128; UNIFIED_ROUND_DEGREE + 1];
    ...
    for i in 0..half {
        ...
        for k in 0..=UNIFIED_ROUND_DEGREE {
            let t = t_flat[k];
    ...
    let evals_tower: [Block128; UNIFIED_ROUND_DEGREE + 1] =
...
//
// Comparison with the per-evaluation reference (`_per_eval`): the per-cell budget
...
fn compute_round_polynomial_flat(
    ...
    let half = tabs.u.len() / 2;
    let mut acc = [0u128; UNIFIED_ROUND_DEGREE + 1]; // 10 coeffs in t
    const ONE_FLAT: u128 = 1u128;
...
// `N_UNIFIED_VARS = 15` rounds. Eleven witness claims share the
...
pub const SHIFT_ROUND_DEGREE: usize = 2;
...
/// Combined output of `prove_unified_sumcheck` + shift gadget. The
...
/// claims.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct FrostCoreProof {
    pub main: UnifiedProof,
    pub shift: ShiftProof,
}

/// Run the shift gadget on the eleven `_dec` claims of an already
/// constructed `UnifiedProof`. The MLE is required because the
...
pub fn prove_shift<T: FiatShamir<Block128>>(
    mle: &UnifiedMle,
    main_red_r_prime: &[Block128],
    channel: &mut T,
) -> (ShiftProof, Vec<Block128>) {
    assert_eq!(main_red_r_prime.len(), N_UNIFIED_VARS);
    ...
    let mut round_polys = Vec::with_capacity(N_UNIFIED_VARS);
    let mut r_double_prime = vec![Block128::ZERO; N_UNIFIED_VARS];
    for round in 0..N_UNIFIED_VARS {
        let poly =
        ...
        r_double_prime[N_UNIFIED_VARS - 1 - round] = r;
        round_polys.push(poly);
    }
...
pub fn verify_shift<T: FiatShamir<Block128>>(
    proof: &ShiftProof,
    main_red: &UnifiedReduction,
    channel: &mut T,
) -> Option<ShiftReduction> {
    if proof.round_polys.len() != N_UNIFIED_VARS {
        return None;
    }
    for p in &proof.round_polys {
        if p.degree() > SHIFT_ROUND_DEGREE {
            return None;
        }
    }
    let delta = channel.squeeze();
...
fn build_combined_weights(r_prime: &[Block128], delta: Block128) -> CombinedWeights {
    ...
    let mut w_dec = vec![Block128::ZERO; N_UNIFIED_CELLS];
    // w_lane[j][x] = eq_slot · eq_round_at_inc(round) · 1[elem==j].
    let mut w_lane: [Vec<Block128>; STATE_SIZE] =
        std::array::from_fn(|_| vec![Block128::ZERO; N_UNIFIED_CELLS]);

    let n_slots = 1usize << N_SLOT_VARS;
    let n_rounds = 1usize << N_ROUND_VARS;
    let n_elems = 1usize << N_LANE_VARS;
    ...
    let mut w_sin = vec![Block128::ZERO; N_UNIFIED_CELLS];
    let mut w_sout = vec![Block128::ZERO; N_UNIFIED_CELLS];
    let mut w_state = vec![Block128::ZERO; N_UNIFIED_CELLS];

    for x in 0..N_UNIFIED_CELLS {
        let dec = w_dec[x];
        w_sin[x] = d0 * dec;
        w_sout[x] = d1 * dec;
        w_state[x] = d2 * dec;
        let elem = x & ((1 << N_LANE_VARS) - 1);
...
fn combined_target(red: &UnifiedReduction, delta: Block128) -> Block128 {
    let d0 = Block128::ONE;
    let d1 = delta;
    let d2 = d1 * delta;
...
fn boolean_tensor(point: &[Block128]) -> Vec<Block128> {
    use frost_gkr_core::mle::eq::eq_ind_partial_eval;
...
/// `flat_shift_round_poly_matches_tower`; the active prover uses
/// `compute_shift_round_polynomial_flat`.
#[allow(dead_code)]
fn compute_shift_round_polynomial(
    ...
    let half = s_in.len() / 2;
    let mut evals = [Block128::ZERO; SHIFT_ROUND_DEGREE + 1];
    for i in 0..half {
...
fn compute_shift_round_polynomial_flat(
    ...
    let half = s_in.len() / 2;
    let mut evals = [0u128; SHIFT_ROUND_DEGREE + 1];
    let t_flat: [u128; SHIFT_ROUND_DEGREE + 1] =
        std::array::from_fn(|k| tower_to_flat_u128(k as u128));
    ...
    let evals_tower: [Block128; SHIFT_ROUND_DEGREE + 1] =
        std::array::from_fn(|k| Block128::from(flat_to_tower_u128(evals[k])));
    RoundPolynomial::from_evals(&evals_tower)
}
...
mod tests {
    use super::*;
    use crate::sequence::N_PERMUTATIONS;
    use crate::unified_mle::build_unified_mle;
    ...
    fn random_mle(seed: u128) -> UnifiedMle {
        let state_ins: Vec<_> = (0..N_PERMUTATIONS)
    ...
    fn round_degree_constant_matches() {
        assert_eq!(UNIFIED_ROUND_DEGREE, 9);
    }

    #[test]
    fn pow7_poly_t_matches_naive() {
        // The 8-coefficient build of (a + t·b)^7 must
    ...
    fn flat_round_poly_matches_tower_round_poly() {
        // The monomial-form builder must match both the tower-basis reference
        // and the per-evaluation flat-basis reference in every round.
        let mle = random_mle(424242);
        let rho: Vec<Block128> = (0..N_UNIFIED_VARS)
    ...
    fn honest_prover_verifies() {
        ...
        let (proof, _r_prime) = prove_unified_sumcheck(&mle, &mut ch_p);
        assert_eq!(proof.round_polys.len(), N_UNIFIED_VARS);

        let mut ch_v = Poseidon2bChannel::new();
        let red = verify_unified_sumcheck(&proof, &mut ch_v).expect("verify must accept");
        assert_eq!(red.r_prime.len(), N_UNIFIED_VARS);
    ...
    fn final_claims_match_native_mle_evaluations() {
        ...
        let mut ch_v = Poseidon2bChannel::new();
        let red = verify_unified_sumcheck(&proof, &mut ch_v).unwrap();
    ...
    fn tampered_state_claim_is_rejected() {
        ...
        let mut ch_p = Poseidon2bChannel::new();
        let (mut proof, _r_prime) = prove_unified_sumcheck(&mle, &mut ch_p);
        proof.state_at_r += Block128::ONE;

        let mut ch_v = Poseidon2bChannel::new();
        assert!(verify_unified_sumcheck(&proof, &mut ch_v).is_none());
    }
    ...
    fn run_full_pipeline(seed: u128) -> (UnifiedMle, FrostCoreProof) {
        let mle = random_mle(seed);
        let mut ch = Poseidon2bChannel::new();
        let (main, r_prime) = prove_unified_sumcheck(&mle, &mut ch);
        let (shift, _r2) = prove_shift(&mle, &r_prime, &mut ch);
        (mle, FrostCoreProof { main, shift })
    }

    #[test]
    fn flat_shift_round_poly_matches_tower() {
        // Check parity between the flat-basis and tower-basis shift builders.
        use rand::Rng;
        let mut rng = rand::thread_rng();
        let n: usize = 1 << N_UNIFIED_VARS;
        let mut make =
    ...
    fn shift_gadget_round_count_and_degree() {
        ...
        assert_eq!(proof.shift.round_polys.len(), N_UNIFIED_VARS);
        for p in &proof.shift.round_polys {
            assert!(p.degree() <= SHIFT_ROUND_DEGREE);
        }
    }
    ...
    fn shift_gadget_verifies_end_to_end() {
        ...
        let mut ch = Poseidon2bChannel::new();
        let main_red = verify_unified_sumcheck(&proof.main, &mut ch).expect("main verify");
        let shift_red = verify_shift(&proof.shift, &main_red, &mut ch).expect("shift verify");
        assert_eq!(shift_red.r_double_prime.len(), N_UNIFIED_VARS);
    }
    ...
    fn shift_final_claims_match_native_evaluations() {
        ...
        let mut ch = Poseidon2bChannel::new();
        let main_red = verify_unified_sumcheck(&proof.main, &mut ch).unwrap();
        let shift_red = verify_shift(&proof.shift, &main_red, &mut ch).unwrap();
    ...
    fn shift_tampered_final_claim_is_rejected() {
        ...
        let mut ch = Poseidon2bChannel::new();
        let main_red = verify_unified_sumcheck(&proof.main, &mut ch).unwrap();
        assert!(verify_shift(&proof.shift, &main_red, &mut ch).is_none());
    }
    ...
    fn shift_combined_target_matches_first_round_sum() {
        ...
        let mut ch_v = Poseidon2bChannel::new();
        let main_red = verify_unified_sumcheck(&main, &mut ch_v).unwrap();
