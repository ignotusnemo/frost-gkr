...
//! Poseidon2b permutation as the layered witness for the preserved
...
//!   from `frost_gkr_poseidon2b::native::permutation`.
//!
//! Partial-round rows zero out lanes 1..3 of `sin`, `x2`,
//! `x4`, `x3`, `sout`, and the MDS_PARTIAL step reads `[sout[0],
//! state[1], state[2], state[3]]` (lanes 1..3 come from the raw state,
//! not from the zeroed S-box outputs).
//!
...
#[cfg(test)]
use frost_gkr_poseidon2b::native::permutation::sbox_x7;
...
/// Byte-identical to the native `permute_mut` branch.
#[inline]
    ...
    for r in 0..N_ROUNDS {
        ...
        let (row_sin, row_x2, row_x4, row_x3, row_sout) = match k {
            ...
            RoundKind::Partial => {
                ...
                // output, lanes 1..3 use the raw state; the witness keeps
                // sin/x2/x3/x4/sout zero on those lanes.
